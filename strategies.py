...
def hand_key(hole_cards):
    ...
    v1, v2 = constants.RANK_VALUES.get(hole_cards[0][:-1], 0), constants.RANK_VALUES.get(hole_cards[1][:-1], 0)
    if v1 < v2:
        r1, r2 = r2, r1
    if r1 == r2:
        return r1 + r2
...
# Analyze the community cards to determine board texture and potential dangers for our hand.
...
def estimate_equity(game_state, player, simulations=600, use_cache=True):
    ...
    for _ in range(simulations):
        ...
        #track used cards per-simulation so opponents can't share hole cards
        used = set(hero_hand + sim_board)
        best_opp_score = None

        for opp in opponents:
            opp_stats = get_opponent_stats(game_state, opp)
            #Pass current used set so each opponent draws from distinct cards
            range_deck = build_range_deck(sim_deck, opp_stats["vpip_pct"], opp_stats["pfr_pct"], excluded_cards=used)
            if len(range_deck) < 2:
                continue
            c1 = range_deck.pop()
            c2 = range_deck.pop()
...
def should_bluff(game_state, player, texture, equity):
    ...
    avg_fold = sum(get_opponent_stats(game_state, opp)["fold_tend"] for opp in active_opps) / max(len(active_opps), 1)

    bluff_prob = 0.08
    if late_pos:
        bluff_prob += 0.10
    if texture['danger'] > 0.5:
        bluff_prob += 0.12
    if avg_fold > 0.55:
        bluff_prob += 0.15
